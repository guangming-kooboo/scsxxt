...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#ApplyStatustablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].ApplyStatusID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteApplyStatus',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#ApplyStatustablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#ApplyStatustablebox').datagrid('loaded');
                                    $('#ApplyStatustablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#ApplyStatustablebox').datagrid({
        url: 'GetTheDataApplyStatus',
        width: 1300,
        title: '申请状态信息表',
        iconCls: 'icon-search',
        nowrap: true,
