...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#Citytablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].CityID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteCity',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#Citytablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#Citytablebox').datagrid('loaded');
                                    $('#Citytablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
