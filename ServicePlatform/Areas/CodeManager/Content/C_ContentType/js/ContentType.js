...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#ContentTypetablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].ConTypeID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteContentType',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#ContentTypetablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#ContentTypetablebox').datagrid('loaded');
                                    $('#ContentTypetablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#ContentTypetablebox').datagrid({
        url: 'GetTheDataContentType',
        width: 1300,
        title: '内容类型信息表',
        iconCls: 'icon-search',
        nowrap: true,
