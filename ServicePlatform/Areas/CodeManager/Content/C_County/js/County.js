...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#Countytablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].CountyID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteCounty',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#Countytablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#Countytablebox').datagrid('loaded');
    ...
    $('#Countytablebox').datagrid({
        url: 'GetTheDataCounty',
        width: 1300,
        title: '县区信息表',
        iconCls: 'icon-search',
        nowrap: true,
