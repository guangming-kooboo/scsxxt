...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#Entagetablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].EntCategoryID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteEnca',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#Entagetablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#Entagetablebox').datagrid('loaded');
                                    $('#Entagetablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#Entagetablebox').datagrid({
        url: 'GetTheDataEntCategory',
        width: 1300,
        title: '企业类别信息表',
        iconCls: 'icon-search',
        nowrap: true,
