...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#Entrtablebox').datagrid('getSelections');//获取选中的要删除的行
                ...
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        for (var i = 0; i < SelectRows.length; i++) {
                            ...
                            Deletedno.push(SelectRows[i].EntRankID);
                        }
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteEntr',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#Entrtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#Entrtablebox').datagrid('loaded');
                                    $('#Entrtablebox').datagrid('load');
    ...
    $('#Entrtablebox').datagrid({
        url: 'GetTheDataEntRank',
        width: 1300,
        title: '企业级别信息表',
        iconCls: 'icon-search',
        nowrap: true,
