...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#NewsTypetablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteNewsType',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#NewsTypetablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#NewsTypetablebox').datagrid('loaded');
                                    $('#NewsTypetablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#NewsTypetablebox').datagrid({
        url: 'GetTheDataNewsType',
        width: 1300,
        title: '新闻类型信息表',
        iconCls: 'icon-search',
        nowrap: true,
