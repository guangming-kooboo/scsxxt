...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#PracAttendanceGradeItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeletePracAttendanceGradeItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#PracAttendanceGradeItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#PracAttendanceGradeItemtablebox').datagrid('loaded');
    ...
    $('#PracAttendanceGradeItemtablebox').datagrid({
        url: 'GetTheDataPracAttendanceGradeItem',
        width: 1300,
        title: '实习考勤条目等级信息表',
        iconCls: 'icon-search',
        nowrap: true,
