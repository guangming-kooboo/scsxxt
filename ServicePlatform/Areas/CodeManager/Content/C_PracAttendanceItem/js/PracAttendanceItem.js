...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#PracAttendanceItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeletePracAttendanceItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#PracAttendanceItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#PracAttendanceItemtablebox').datagrid('loaded');
    ...
    $('#PracAttendanceItemtablebox').datagrid({
        url: 'GetTheDataPracAttendanceItem',
        width: 1300,
        title: '实习考勤条目信息表',
        iconCls: 'icon-search',
        nowrap: true,
