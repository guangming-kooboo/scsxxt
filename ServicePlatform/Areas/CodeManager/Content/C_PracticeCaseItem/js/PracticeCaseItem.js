...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#PracticeCaseItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeletePracticeCaseItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#PracticeCaseItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#PracticeCaseItemtablebox').datagrid('loaded');
                                    $('#PracticeCaseItemtablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#PracticeCaseItemtablebox').datagrid({
        url: 'GetTheDataPracticeCaseItem',
        width: 1300,
        title: '实习案例信息表',
        iconCls: 'icon-search',
        nowrap: true,
