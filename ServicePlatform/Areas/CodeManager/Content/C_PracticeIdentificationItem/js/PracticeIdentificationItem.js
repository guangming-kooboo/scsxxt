...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#PracticeIdentificationItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeletePracticeIdentificationItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#PracticeIdentificationItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#PracticeIdentificationItemtablebox').datagrid('loaded');
                                    $('#PracticeIdentificationItemtablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#PracticeIdentificationItemtablebox').datagrid({
        url: 'GetTheDataPracticeIdentificationItem',
        width: 1300,
        title: '实习鉴定信息表',
        iconCls: 'icon-search',
        nowrap: true,
