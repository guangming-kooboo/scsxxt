...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#StuEvaEntGradeLevelItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteStuEvaEntGradeLevelItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#StuEvaEntGradeLevelItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#StuEvaEntGradeLevelItemtablebox').datagrid('loaded');
                                    $('#StuEvaEntGradeLevelItemtablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#StuEvaEntGradeLevelItemtablebox').datagrid({
        url: 'GetTheDataStuEvaEntGradeLevelItem',
        width: 1300,
        title: '学生评价企业等级等级信息表',
        iconCls: 'icon-search',
        nowrap: true,
