...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#StuEvaSchoolGradeLevelItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteStuEvaSchoolGradeLevelItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#StuEvaSchoolGradeLevelItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#StuEvaSchoolGradeLevelItemtablebox').datagrid('loaded');
    ...
    $('#StuEvaSchoolGradeLevelItemtablebox').datagrid({
        url: 'GetTheDataStuEvaSchoolGradeLevelItem',
        width: 1300,
        title: '学生评价学校等级信息表',
        iconCls: 'icon-search',
        nowrap: true,
