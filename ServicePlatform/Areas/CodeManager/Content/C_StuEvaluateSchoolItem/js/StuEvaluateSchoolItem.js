...
$(function () {
    ...
    obj = {
        ...
        remove: function () {
            //删除按钮
            var SelectRows = $('#StuEvaluateSchoolItemtablebox').datagrid('getSelections');//获取选中的要删除的行
            if (SelectRows.length > 0) {
                $.messager.confirm('确定操作', '你确定要删除选中的数据吗？', function (flag) {
                    if (flag) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/CodeManager/Code/DeleteStuEvaluateSchoolItem',
                            data: {
                            ...
                            beforeSend: function () {
                                $('#StuEvaluateSchoolItemtablebox').datagrid('loading');
                            },
                            success: function (data) {
                                if (data) {
                                    $('#StuEvaluateSchoolItemtablebox').datagrid('loaded');
                                    $('#StuEvaluateSchoolItemtablebox').datagrid('load');
                                    $.messager.show({
                                        title: '提示信息',
                                        msg: data + "个种类被删掉了",
                                    });
    ...
    $('#StuEvaluateSchoolItemtablebox').datagrid({
        url: 'GetTheDataStuEvaluateSchoolItem',
        width: 1300,
        title: '学生评价学校条目信息表',
        iconCls: 'icon-search',
        nowrap: true,
